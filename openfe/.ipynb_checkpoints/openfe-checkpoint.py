...
from .random_importance import get_random_importance
...
def _subsample(iterators, n_data_blocks):
    # 将输入的迭代器转换为列表，以便进行随机操作
    iterators = list(iterators)
    # 计算每个数据块的大小，即总迭代器数除以数据块数量
    length = int(len(iterators) / n_data_blocks)
    # 随机打乱迭代器列表，以确保随机性
    random.shuffle(iterators)
    # 初始化结果列表，第一个元素为前length个迭代器
    ...
    while n_data_blocks != 1:
        ...
        # 更新每个数据块的大小，是上一次的两倍
        length = int(length * 2)
        # 如果数据块数量变为1，则将剩余的所有迭代器添加到结果列表
        if n_data_blocks == 1:
            results.append(iterators)
        else:
            # 否则，将前length个迭代器添加到结果列表
...
class OpenFE:
    ...
    def fit(
        ...
        assert stage2_metric in ["gain_importance", "permutation", "random_importance"]
    ...
    def stage2_select(self):
        ...
        if self.stage2_metric == "gain_importance":
            ...
            # r.to_csv("permutation.csv")
            for i, imp in enumerate(r.importances_mean[: len(new_features)]):
                results.append([formula_to_tree(new_features[i]), imp])
        elif self.stage2_metric == "random_importance":
            r = get_random_importance(
            ...
            # r.to_csv("random_importance.csv")
            for i, imp in enumerate(r[: len(new_features)]):
                results.append([formula_to_tree(new_features[i]), imp])
        return results
    ...
    def _calculate_and_evaluate_multiprocess(
        ...
        try:
            ...
            # train_init.to_csv("train_init.csv")
            # val_init.to_csv("val_init.csv")
